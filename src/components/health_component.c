...
void PlayerDeath(struct Component *selfComponent, struct Game *game)
{
    ...
    if (healthComponent->currentLives <= 0) //final life spent, goodbye my friend.
    {
        ...
        struct Component *tb = AddComponent(selfComponent->owner, TimedBehaviourC, GoToMainMenuAfter, game->engine->ECS);
    ...
    struct Component *tb = AddComponent(selfComponent->owner, TimedBehaviourC, PlayerTimedRespawnBehaviour, game->engine->ECS);
    InitTimedBehaviourComponent(tb->data, 1, 5.0f, NULL);

    tb = AddComponent(selfComponent->owner, TimedBehaviourC, AlphaBlendingBehaviour, game->engine->ECS);
...
void EnemyDeath(struct Component *selfComponent, struct Game *game)
{
    ...
    if (lifePowerUpExtraction <= bld->lifePowerupSpawnChance)
    {
        ...
        InitTransformComponent(GetComponentData(powerup, TransformC), transformComponent->position, vec2_new(0,0));
...
void BossDeath(struct Component *selfComponent, struct Game *game)
{
    ...
    c = AddComponent(selfComponent->owner, TimedBehaviourC, BossExplosionBehaviour, game->engine->ECS);
