...
void InitPhysicsComponent(struct Component *selfComponent, void (*Collide)(struct Component *selfComponent, struct Component *otherComponent, struct Game *game), boolean isTrigger, float colliderRadius)
{
    ...
    {
        if (colliderRadius > 0)
            physicsComponent->colliderRadius = colliderRadius;
        else
        {
            struct TransformComponent* tc = GetComponentData(selfComponent->owner, TransformC);
            vec2 colliderScale = vec2_mul_scal(&tc->scale, 2.0f);
            physicsComponent->colliderRadius = vec2_magn(&colliderScale);
        }
    }
    else
    ...
    case Airplane: //i am an airplane
        ...
        case PlayerAirplane:                                    //specifically, i am a player airplane
            ...
            else if (otherComponent->owner->family == AttackPowerup) //other is an attack pu
            {
                ...
                struct Component *tb = AddComponent(selfComponent->owner, TimedBehaviourC, DetachAttackPowerupBehaviour, game->engine->ECS);
            ...
            else if (otherComponent->owner->family == SpeedPowerup) //other is a speed pu
            {
                ...
                struct Component *tb = AddComponent(selfComponent->owner, TimedBehaviourC, DetachSpeedPowerupBehaviour, game->engine->ECS);
                InitTimedBehaviourComponent(tb->data, 1, bld->powerupDuration, NULL);
...
void PhysicsBehaviour(struct Component *selfComponent, struct Game *game)
{
    ...
    aiv_vector *physicsSystem = GetSystem(game->engine->ECS, PhysicsC);
